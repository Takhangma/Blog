...
namespace CourseWork.Modules.User.Services
{
    public class UserService
    {
        ...
        public async Task<UserEntity> CreateUser(UserCreateDto data)
        {
            ...
            await _emailService.SendVerificationEmail(data.Email, data.UserName, $"https://localhost:7055/api/user/auth/verify-email/{WebUtility.UrlEncode(data.Email)}");
        ...
        {
            await _emailService.SendVerificationEmail(user.Email, user.Name, $"https://localhost:7055/api/user/auth/verify-password-change/{WebUtility.UrlEncode(user.Email)}");
            return true;
        }
